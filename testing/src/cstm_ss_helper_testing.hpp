#ifndef cstm_ss_helper_testing_hpp
#define cstm_ss_helper_testing_hpp


#include "liborangepower_src/misc_includes.hpp"
#include "liborangepower_src/misc_output_classes.hpp"

#include "liborangepower_src/range_funcs.hpp"

using liborangepower::printout_etc::printout;
using liborangepower::printout_etc::printerr;

#include "cstm_ss_helper_classes.hpp"


void test_cstm_ss_helper_ne(const string_view& to_test);
void test_cstm_ss_helper_we(const string_view& to_test);


#endif		// cstm_ss_helper_testing_hpp

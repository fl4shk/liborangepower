...
#define LIBOP_INNER_MAKE_DEFERRED_RESTORER_CLASS_CTOR(type, var_name) \
	_saved_ ## var_name = std::move(self-> var_name )

#define LIBOP_INNER_MAKE_DEFERRED_RESTORER_CLASS_DTOR(type, var_name) \
...
#define MAKE_DEFERRED_RESTORER_CLASS(name, self_t, ...) \
	...
	protected:		/* variables */ \
		...
		EVAL(MAP_PAIRS(LIBOP_INNER_MAKE_DEFERRED_RESTORER_CLASS_MEMB_VAR, \
			SEMICOLON, __VA_ARGS__)); \
	public:		/* functions */ \
		inline name (self_t* s_self) \
			: _self(s_self) { \
			EVAL(MAP_PAIRS(LIBOP_INNER_MAKE_DEFERRED_RESTORER_CLASS_CTOR, \
				SEMICOLON, __VA_ARGS__)); \
		} \
		virtual inline ~ name () { \
			EVAL(MAP_PAIRS(LIBOP_INNER_MAKE_DEFERRED_RESTORER_CLASS_DTOR, \
				SEMICOLON, __VA_ARGS__)); \
		} \
	}
